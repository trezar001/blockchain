...
if(command == 'mine'){
    ...
    let amount = Number.parseFloat(process.argv[process.argv.indexOf('-a') + 1]);
    socket.emit('enough', {amount: amount, address: wallet});
    socket.on('enough', (res)=>{
        if(res){
            ...
            console.log('[-] You do not have enough in your wallet to make this transaction!')
        }
    })
}
